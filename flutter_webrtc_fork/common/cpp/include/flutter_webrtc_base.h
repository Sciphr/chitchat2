...
#ifdef _WIN32
  // Maps audio track ID → active WASAPI loopback capture instance.
  // Stored as shared_ptr<void> to avoid a Windows-only header dependency here;
  // callers cast to WasapiLoopbackCapture*.
  std::map<std::string, std::shared_ptr<void>> loopback_capturers_;
#endif
