  ...
  for (auto it : base_->local_streams_) {
    ...
    for (auto track : audio_tracks.std_vector()) {
      if (track->id().std_string() == track_id) {
        stream->RemoveTrack(track);
#ifdef _WIN32
        // Stop WASAPI loopback capture tied to this audio track (if any).
        auto loopback_it = base_->loopback_capturers_.find(track_id);
        if (loopback_it != base_->loopback_capturers_.end()) {
          auto loopback = std::static_pointer_cast<WasapiLoopbackCapture>(
              loopback_it->second);
          loopback->Stop();
          base_->loopback_capturers_.erase(loopback_it);
        }
#endif
      }
    }
