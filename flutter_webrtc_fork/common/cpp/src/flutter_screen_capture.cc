...
#ifdef _WIN32
#include "../../../windows/wasapi_loopback_capture.h"
...
void FlutterScreenCapture::GetDisplayMedia(
  ...
  // Check whether the caller requested system audio capture.
  ...
  if (capture_audio) {
    ...
    if (audio_source) {
      ...
      if (audio_track) {
        // Start the WASAPI loopback capture feeding into this source.
        auto loopback = std::make_shared<WasapiLoopbackCapture>(audio_source);
        if (loopback->Start()) {
          base_->loopback_capturers_[audio_track->id().std_string()] =
              loopback;
